...
import { fetchSettingByParam, fetchCurrentUser, updateSetting } from '@/app/lib/fetch';
// @ts-ignore
...
export default function EditForm({ id }: { id: string }) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const sendResponse = await updateSetting(formData, editId);
  ...
  useEffect(() => {
    const fetchCategoryDataByID = async () => {
      setLoading(true);
      try {
        const fetchedData = await fetchSettingByParam(editId);
        setFormData({
          param: fetchedData.param,
