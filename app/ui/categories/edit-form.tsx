...
import { fetchCategoryByID, fetchCurrentUser, updateCategory } from '@/app/lib/fetch';
// @ts-ignore
...
export default function EditForm({ id }: { id: string }) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const sendResponse = await updateCategory(formData, editId);
  ...
  useEffect(() => {
    const fetchCategoryDataByID = async () => {
      setLoading(true);
      try {
        const fetchedData = await fetchCategoryByID(editId);
        setFormData({
