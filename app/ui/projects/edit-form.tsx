...
import { fetchAllTechs, fetchProjectByID, fetchCurrentUser, updateProject } from '@/app/lib/fetch';
// @ts-ignore
...
export default function EditForm({ id }: { id: string }) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const sendResponse = await updateProject(formData, editId);
  ...
  useEffect(() => {
    ...
    const fetchProjectDataByID = async () => {
      ...
      try {
        ...
        initTechsInput(techList);

        setFormData({
          title: fetchedData.title,
          content: fetchedData.content,
          source: fetchedData.source ? fetchedData.source : null,
          url: fetchedData.url ? fetchedData.url : null,
          demo: fetchedData.demo ? fetchedData.demo : null,
          relevant: fetchedData.relevant,
  ...
  const initTechsInput = (techList: number[]) => {
    let itemList = document.querySelectorAll<HTMLLIElement>("#tech-list li");
    let techsText: TechSet[] = [];
    itemList.forEach((e) => {
      let dataId = e.getAttribute("data-id");
      if(dataId){
        let parseId = parseInt(dataId);
        if(techList.includes(parseId)){
          techsText.push({
            id: parseId,
            name: e.innerHTML
          });
